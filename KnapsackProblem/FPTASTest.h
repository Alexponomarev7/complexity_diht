...
namespace fptas {
    ...
    Test generate_simple_FPTAS_test(const TestCase<Key, Value> &test_case_data) {
        Test test;

        long long n = 4;
        long long knapsack_capacity = 5;
        std::vector<long long> weights{2, 4, 1, 2};
        ...
        double answer = solve_knapsack(weights, costs, knapsack_capacity);
        test << answer;
    };

    template<class Key, class Value>
    Test generate_random_FPTAS_test(const TestCase <Key, Value> &test_case_data) {
        Test test;
        ...
        double value = fake / 2 + (rand() % 99 + 1) * step;

        std::vector<double> costs{value, value, fake};
